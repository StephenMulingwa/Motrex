...
/** Vercel cron: 04:00 UTC daily = 07:00 EAT — previous day's fleet utilization. */
...
export async function GET(request: Request) {
  ...
  const runId = await cronRunStart("daily-utilization");
  const reportDate = yesterdayEatDateString();
  const details: Record<string, unknown> = { reportDate, reportType: "utilization" };
  let ok = true;

  try {
    await ensureSchema();
    const result = await withWialonRetry(`${reportDate} / utilization`, () =>
      executeStoredReport("utilization", reportDate),
    );
    await upsertReportSnapshot(result);
    details.rowCount =
      result.payload.rows?.length ?? Object.keys(result.payload.pivot ?? {}).length;
    details.vehicleCount = (result.payload.summary as { vehicleCount?: number } | undefined)
      ?.vehicleCount;
